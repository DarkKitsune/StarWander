...
const vec4 COLOR_RIM = vec4(1, 1, 0.2, 1);
const vec4 COLOR_LIGHT = vec4(1, 1, 0, 1);
const vec4 COLOR_DARK = vec4(1, 0.2, 0, 1);

// Threshold at which the "dark" (opposite of the planet base color) color is used
const float DARK_THRESHOLD = 0.5;
...
void main()
{
	...
	// Mix dark spots
	float darkSample = noise_sample((-varying_vPos / Radius) * 50);
	color = mix(color, COLOR_DARK, darkSample);

	// Mix rim lighting
	float rimLighting = min(1, 1 + dot(varying_vDirectionFromCam, varying_vNorm));
	rimLighting = rimLighting * rimLighting;
	color = mix(color, COLOR_RIM, rimLighting);

	out_fColor = color * modelColor();
}